...
const getSvnUrl = async (path = process.cwd(), buildType = ''): Promise<svnUrlDataType> => {
  return new Promise((resolve, reject) => {
    svnCommands.commands.info(path, (err: any, data: any) => {
      ...
      const svnUrlData: svnUrlDataType = {
        url: decodeURI(data.entry.url),
        relativeUrl: decodeURI(data.entry['relative-url']),
        buildType: buildType
      }

      if (svnUrlData.buildType !== '') {
        resolve(svnUrlData)
      } else {
        const buildTypeArr = [
          {
        ...
        if (isValid) {
          ...
          logError('此路径不是有效的可构建路径,目前仅仅支持构建课件 / egret / cocos.')
          console.log(Colors.red(('此路径不是有效的可构建路径,目前仅仅支持构建课件 / egret / cocos / h5.')));
          process.exit()
        }
      }
    })
